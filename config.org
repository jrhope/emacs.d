...
MELPA doesn't have frame-cmds anymore.  Looks like now I should use
Quelpa instead: [[https://www.reddit.com/r/emacs/comments/8i0o0m/zoomfrm/][zoom-frm : emacs]].
...
#+begin_src emacs-lisp
  (use-package csv-mode :ensure t :mode ("\\.csv\\'" . csv-mode))
#+end_src
...
#+BEGIN_SRC emacs-lisp
  ...
  CONTENTS is nil.  INFO is a plist holding contextual information."
    ...
    (defun jrh/org-md-latex-fragment (latex-fragment _contents info)
      (let ((res (org-html-latex-fragment latex-fragment _contents info)))
        (if (and res (string-match "\\\\(\\(.*\\)\\\\)" res))
            (format "\\f$%s\\f$" (match-string 1 res))
          res)))
    (push '(latex-fragment . jrh/org-md-latex-fragment)
          (org-export-backend-transcoders (org-export-get-backend 'md)))

    (defun jrh/org-md-latex-environment (latex-environment contents info)
      "Transcode a LATEX-ENVIRONMENT element from Org to Markdown.
  CONTENTS is nil.  INFO is a plist holding contextual information."
      (let ((res (org-html-latex-environment latex-environment contents info)))
        (when (and res (string-match "^\\\\begin{\\(.*\\)}\\(.*\\(?:\n.*\\)*?\\)\\\\end{\\1}\n$" res))
          (setq res (concat "\\f{" (match-string 1 res) "}{" (match-string 2 res) "\\f}\n")))
        (message "latex-environment:\n'%s'" res)
        res))
    (push '(latex-environment . jrh/org-md-latex-environment)
          (org-export-backend-transcoders (org-export-get-backend 'md)))

    (defun jrh/org-md-link (link contents info)
      (let ((res (org-md-link link contents info)))
        (if (and res (string-match "!\\[img\\](\\(.*\\))" res))
            (let ((txt (match-string 1 res))
                  (num (org-export-get-ordinal link info nil #'org-html-standalone-image-p)))
              (if (string-match "\\(.*\\) \"\\(.*\\)\"" txt)
                  (let* ((file (match-string 1 txt))
                         (cap-raw (match-string 2 txt))
                         (cap-fix (replace-regexp-in-string "\\\\_" "_" cap-raw))
                         (cap (format "Figure %s: %s" num cap-fix)))
                    (format "![%s](%s \"%s\")" cap file cap))
                (format "![Figure %s](%s \"Figure %s\")" num txt num)))
          (org-html-link link contents info))))
    (push '(link . jrh/org-md-link)
          (org-export-backend-transcoders (org-export-get-backend 'md)))

    (defun jrh/org-md-paragraph (paragraph contents info)
      (let ((res (org-md-paragraph paragraph contents info)))
        (if (org-html-standalone-image-p paragraph info)
            (let ((label (and (org-element-property :name paragraph)
                              (org-export-get-reference paragraph info))))
              (if (org-string-nw-p label)
                  (org-html--wrap-image res info nil label)
                res))
          res)))
    (push '(paragraph . jrh/org-md-paragraph)
          (org-export-backend-transcoders (org-export-get-backend 'md)))
    ...
    (defun jrh/org-md-table-row (table-row contents info)
      (if (org-export-table-row-ends-header-p table-row info)
          (concat contents "\n"
                  (mapconcat 'identity
                             (mapcar '(lambda (s) (make-string (length s) ?-))
